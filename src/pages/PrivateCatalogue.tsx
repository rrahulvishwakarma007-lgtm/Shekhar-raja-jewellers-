...
export default function PrivateCatalogue() {
  ...
  const [customProducts, setCustomProducts]   = useState<NewProduct[]>(() => {
    try { return JSON.parse(localStorage.getItem('srj_custom_products') || '[]'); }
    catch { return []; }
  });
  ...
  const allItems = [...catProducts, ...uploadedAsProducts, ...customProducts];
  ...
  const handleAddProduct = (_catKey: string, product: NewProduct, stock: StockChoice) => {
    const updated = [...customProducts, product];
    setCustomProducts(updated);
    try { localStorage.setItem('srj_custom_products', JSON.stringify(updated)); } catch {}
    if (stock === 'ordered') {
      moveToOrdered(product.id);
      setStockMap(prev => ({ ...prev, [product.id]: 'ordered' }));
    }
    setOrderedToast(`${product.name} added to ${stock === 'ready' ? 'Ready' : 'Ordered'} Stock!`);
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-full font-cinzel text-[10px] tracking-[0.12em] text-white transition-all"
              style={{ background: `linear-gradient(135deg, ${C.gold}, ${C.goldDk})`, boxShadow: `0 3px 12px rgba(194,24,91,0.3)` }}>
              <Plus size={13} />
              Add Product
            </motion.button>
            {/* Timer */}
                ...
                return (
                  <motion.div key={product.id} layout variants={itemVariants}
                    ...
                    <div className="p-4 sm:p-5 flex-1 flex flex-col">
                      ...
                      {/* Weight / Material / Karate specs (custom products) */}
                      {(product.weight || product.material || product.karate) && (
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {product.weight && (
                            <span className="font-cinzel text-[8px] tracking-[0.08em] px-2 py-0.5 rounded-full"
                                  style={{ background: 'rgba(194,24,91,0.07)', color: C.pink }}>
                              {product.weight}
                            </span>
                          )}
                          {product.karate && (
